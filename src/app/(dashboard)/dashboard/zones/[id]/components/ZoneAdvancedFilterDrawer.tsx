...
export function ZoneAdvancedFilterDrawer() {
  ...
  const handleApply = () => {
    ...
    const FILTER_KEYS = ['christian_name', 'age_min', 'age_max', 'gender', 'status', 'marital_status', 'page'];
  ...
  const handleClear = () => {
    setLocalFilters({
      ...
      gender: '', status: [], marital_status: '',
    });
    const params = new URLSearchParams(searchParams.toString());
    ['christian_name', 'age_min', 'age_max', 'gender', 'status', 'marital_status', 'page'].forEach((k) => params.delete(k));
  ...
  return (
    <>
      ...
      <button
        ...
        className={`h-10 px-4 flex items-center gap-2 border text-sm font-medium rounded transition-all active:scale-95 focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-1 shrink-0 ${
          hasActiveFilters
      ...
      {isOpen && (
        <div
          className="fixed inset-0 z-[60] bg-[#1C1917]/40 backdrop-blur-[2px]"
      ...
      <div
        ...
        className={`fixed top-0 right-0 bottom-0 z-[70] w-full md:w-[400px] bg-surface border-l border-[#E7E5E4] flex flex-col shadow-2xl transition-transform duration-300 ease-in-out ${
